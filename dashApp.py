...
navbar = dbc.NavbarSimple(
    children=[
        ...
        dbc.NavItem(html.A("World Data",href = "#world_overall_data",style={'color': '#fff'}), className="mr-5" ),
...
main_heading = dbc.Container(
    [
        html.H1(["COVID-19 Analysis Dashboard"], className="my-5 pt-5 text-center"),
    ]
    , className='pt-3')
...
world_data_heading = html.H2(id='world_overall_data',children = " Overall Cases around the world", className='mt-5 pb-3 text-center')
...
##########Pie Plot for total cases across world################

pie_plot=dbc.Container(children=[
     html.Div([
...
def plot_world_info(caseType):
    if(caseType == 'confirmed'): # defining the trace for the pie plot for confirmed cases
        trace = go.Pie(
            ...
            values = country_df.Confirmed, # using the confirmed case values from the data set for pie plot
            hole = 0.6,
        ...
        trace = go.Pie(
            name = str(caseType) + ' cases',
            labels = country_df.Country,
            values = country_df.Active, # using the active case values from the data set for pie plot
            hole = 0.6,
            hoverinfo = 'label+value+percent', #defining the text to be displayed on mouse tool-tip when hovered
            textposition = 'inside'
        )
        ...
        trace = go.Pie(
            name = str(caseType) + ' cases',
            labels = country_df.Country,
            values = country_df.Deaths, # using the death case values from the data set for pie plot
            hole = 0.6,
            hoverinfo = 'label+value+percent', #defining the text to be displayed on mouse tool-tip when hovered
            textposition = 'inside'
        )
        ...
        trace = go.Pie(
            name = str(caseType) + ' cases',
            labels = country_df.Country,
            values = country_df.Recovered, # using the recovered case values from the data set for pie plot
            hole = 0.6,
            hoverinfo = 'label+value+percent', #defining the text to be displayed on mouse tool-tip when hovered
            textposition = 'inside'
        )
